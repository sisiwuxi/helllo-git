...
void GetTotalAverage(void)
{
	...
	AverageValue.TotalAverage=sum/(HEIGHT*WIDTH);
	sum=0;	
	k=0;
	for(j=0; j<HEIGHT; j++)
	{
		for(i=0; i<WIDTH; i++)
		{	
			if(ARRAY_Y[j][i]>=AverageValue.TotalAverage)
			{
				sum+=ARRAY_Y[j][i];
				k++;
			}
...
void ThirdAllSeed(void)
{
	...
	Max=ARRAY_Y[0][0];	
	Min=ARRAY_Y[0][0];	
	Seeds[1].value=ARRAY_Y[0][0];//max
	Seeds[4].value=ARRAY_Y[0][0];//min	
	for(j=0; j<HEIGHT; j++)
	{
		for(i=0; i<WIDTH; i++)
		{	
			if((ARRAY_Y[j][i]>Max) && !(Seeds[0].x==j && Seeds[0].y==i))			
			{
				Seeds[1].x=j;
				Seeds[1].y=i;
				Max=ARRAY_Y[j][i];
				Seeds[1].value=ARRAY_Y[j][i];
			}
			if((ARRAY_Y[j][i]<Min) && !(Seeds[3].x==j && Seeds[3].y==i))			
			{
				Seeds[4].x=j;
				Seeds[4].y=i;
				Min=ARRAY_Y[j][i];
				Seeds[4].value=ARRAY_Y[j][i];
			}			
		}
	}	
	...
	Max=ARRAY_Y[0][0];	
	Min=ARRAY_Y[0][0];	
	Seeds[2].value=ARRAY_Y[0][0];//max
	Seeds[5].value=ARRAY_Y[0][0];//min		
	for(j=0; j<HEIGHT; j++)
	{
		for(i=0; i<WIDTH; i++)
		{	
			if((ARRAY_Y[j][i]>Max) && !(Seeds[0].x==j && Seeds[0].y==i) && !(Seeds[1].x==j && Seeds[1].y==i))				
			{
				Seeds[2].x=j;
				Seeds[2].y=i;
				Max=ARRAY_Y[j][i];
				Seeds[2].value=ARRAY_Y[j][i];
			}
			if((ARRAY_Y[j][i]<Min) && !(Seeds[3].x==j && Seeds[3].y==i) && !(Seeds[4].x==j && Seeds[4].y==i))				
			{
				Seeds[5].x=j;
				Seeds[5].y=i;
				Min=ARRAY_Y[j][i];
				Seeds[5].value=ARRAY_Y[j][i];
			}			
		}
	}		
	LOG_SEEDS();
...
void DifferenceCalculation(void)
{
	...
	for(k=0;k<SEED_NUM;k++)
	{
		for(j=0; j<HEIGHT; j++)
		{
			for(i=0; i<WIDTH; i++)
			{			
				Filters[k].ARRAY_Difference[j][i]=Seeds[k].value-ARRAY_Y[j][i];
			}
		}
	}
	LOG_FILTERS_ARRAY();	
...
void TotalAveageFilter(void)
{
	...
	for(k=0; k<SEED_NUM/2; k++)//MAX
	{
		...
		Filters[k].AverageFilter[Seeds[k].x][Seeds[k].y]=2;
	}	
	
	
	for(k=SEED_NUM/2; k<SEED_NUM; k++)//MIN
	{
		for(j=0; j<HEIGHT; j++)
		{
			for(i=0; i<WIDTH; i++)
...
void DropStone(int num)
{
	...
	while(!feof(fp1))
	{
		...
		if(Coefficient!=0)
		{
			...
			printf("(%d,%d) ",Coefficient, CoefficientTest);
			if(Coefficient>0)
			{
