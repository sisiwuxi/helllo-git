...
hello_msg = "Welcom to Coach Kelly's Timing App"
list_title = 'Here is your list of athletes:'
quit_msg = "Quitting Coach Kelly's App."

web_server = 'http://192.168.1.7:8080'
...
def send_to_server(url, post_data=None):
	if post_data:
		page = urlopen(url, urlencode(post_data).encode('utf8'))
	else:
		page = urlopen(url)
...
if resp['which'] in ('positive'):
	...
	athlete_title = str(athlete['Name']) + '(' + str(athlete['DOB']) + '), top 3 times:'
	app.dialogCreateAlbert(athlete_title)
	#app.dialogSetItems(str(athlete['TOP3']))
